		...
		if(scanf("%d", &sel) == 1){
			...
			if(sel > 0 && sel < 6){
				...
				// ---------------------------- COMPRAR PRODUCTOS (ELIMINAR) --------------------------------------------
				 if (sel == 3) {
				    	char nombreCompra[30]; // Se define una variable para almacenar el nombre de compra
				    	printf("\nIngrese el nombre del producto a comprar: ");
				    	scanf("%s", nombreCompra);
				    	comprarProducto(productos, &numProductos, nombreCompra); // Se llama una función para que inicie la compra
				 }
