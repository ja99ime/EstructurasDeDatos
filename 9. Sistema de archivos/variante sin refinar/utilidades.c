...
NODO * mover(NODO *actual, char dato[]) {
	
    	if (actual == NULL) {
        	return NULL;  									// Si el nodo actual es NULL, no podemos movernos
    	}

    	if (actual->hijo_izq != NULL && strcmp(actual->hijo_izq->nombre, dato) == 0) {
        	if (actual->hijo_izq->esArchivo == 1){
        		printf("[ERROR] Esto no es una carpeta.\n");
        	} else return actual->hijo_izq;  							// Retorna el hijo izquierdo si coincide con dato
        	
    	} else if (actual->hijo_der != NULL && strcmp(actual->hijo_der->nombre, dato) == 0) {
        	if (actual->hijo_der->esArchivo == 1){
        		printf("[ERROR] Esto no es una carpeta.\n");
        	} else return actual->hijo_der;  							// Retorna el hijo derecho si coincide con dato
    	}
