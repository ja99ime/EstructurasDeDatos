...
nodo * insertarNodo(nodo **inicio, nodo *refe, struct Movil producto){
	...
	// Se asginan los valores del prodcuto al nuevo nodo
	nuevo->productos = producto;
    
    	// Si la lista está vacía, el nuevo nodo sera el primero
    	if (listaVacia(*inicio) == TRUE){ 
        	*inicio = nuevo;
    	} else { 
        	// El nuevo producto se insertara al principio si tiene un código menor al inicio
        	if (producto.codigo < (*inicio)->productos.codigo) {
            		nuevo->siguiente = *inicio;
            		*inicio = nuevo;
        	} else { 
        		// En caso contrario, se recorrerar los elementos del vector para dar con su posición
            		nodo *actual = *inicio;
            		// Se buscara la posicion correcta para insertrar el nuevo nodo
            		while (actual->siguiente != NULL && producto.codigo > actual->siguiente->productos.codigo) {
                		actual = actual->siguiente;
            		}
            		// UNa vez encontra la posición, se insertará en el lugar correcto.
