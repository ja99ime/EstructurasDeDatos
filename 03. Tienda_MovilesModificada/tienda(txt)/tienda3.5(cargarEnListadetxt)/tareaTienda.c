...
int main(){
	...
	nodo *inicio = NULL;							// Establecemos uns nueva lista
	int numProductos = cargarProductosArray(productos, &inicio);	 	// El numero de productos será igual a los registros que haya en el fichero
	printf("Productos registrados en el sistema: %d\n", numProductos); 	// Se informa al usuario del numero de elementos que hay
	

	nodo *insertado;							// Establecemos una referencia por si acaso necesitamos insertar un nuevo valor
	mostrarLista(inicio);							// COMPROBANTE
