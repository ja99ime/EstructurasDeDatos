...
nodo * insertarNodo(nodo **inicio, nodo *refe, struct Movil producto){
		...
		printf("\n[ERROR]No se ha podido reservar espacio para el nuevo elemento de la lista");
		return NULL;
    	}
	...
	nuevo->productos = producto;
    	...
    	if (listaVacia(*inicio) == TRUE){ 
        	*inicio = nuevo;
        	...
        	// Si el nuevo producto debe insertarse al principio porque tiene un código menor
        	if (producto.codigo < (*inicio)->productos.codigo) {
            		nuevo->siguiente = *inicio;
            		*inicio = nuevo;
        	} else { 
            		nodo *actual = *inicio;
            		// Buscar la posición correcta para insertar el nuevo nodo
            		while (actual->siguiente != NULL && producto.codigo > actual->siguiente->productos.codigo) {
...
int cargarProductos(struct Movil productos[]) {
	...
	if (file == NULL) { // Nos aseguramos de que se abre. Si no, informamos al usuario
		printf("[ERROR]No se pudo abrir el archivo. Comenzando con un inventario vacío.\n");
		return 0;
	}

	int numProductos = 0; // El numero de productos se establece incialmente a 0 
	while (fscanf(file, "%d %s %s %s %f %s %s %s %f %d %d",
		&productos[numProductos].codigo, productos[numProductos].nombre, productos[numProductos].descripcion, productos[numProductos].estado,
...
void guardarProductos(struct Movil productos[], int numProductos) {
    ...
    for (int i = 0; i < numProductos; i++) { // Uno a uno, se copian todos los valores del array de prodcutos en el fichero
        fprintf(file, "%d %s %s %s %.2f %s %s %s %.2f %d %d\n",
                &productos[i].codigo, productos[i].nombre, productos[i].descripcion, productos[i].estado,
